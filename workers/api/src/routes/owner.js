...
async function ownedGround(env, ctx, idOrSlug) {
  const ref = str(idOrSlug, 'id', { required: true, max: 120 });
  const filter = UUID_RE.test(ref)
    ? `id=eq.${ref}`
    : `slug=eq.${encodeURIComponent(ref.toLowerCase())}`;
  const row = await sbMaybeOne(
    env,
    `grounds?select=${GROUND_COLS}&${filter}&owner_id=eq.${ctx.userId}&limit=1`,
    { token: ctx.token },
  );
